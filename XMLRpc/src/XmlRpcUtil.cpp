...
// Default log handlerrpc
static class DefaultLogHandler : public XmlRpcLogHandler {
public:
	DefaultLogHandler(){
		m_hMux = CreateMutex(NULL, FALSE, "_XML_RPC_LOCK_");
	}
...
#else
		if (level <= _verbosity){
			/*wait for 2s to avoid threading exception dead without unlock*/
			WaitForSingleObject(m_hMux, 2000);
...
XmlRpcLogHandler* XmlRpcLogHandler::_defLogHandler = &defaultLogHandler;
...
{
	/*DefLogHdler - CurLogHdler*/
	va_list va;
	char buf[1024];

	if(XmlRpcLogHandler::getDefLogHanler() != XmlRpcLogHandler::getLogHandler()){
		va_start( va, fmt);
		...
		{
			XmlRpcLogHandler::getDefLogHanler()->log(level, buf);
		}
		return;
	}
